...
function goToProcess(processId){
    window.location = window.location.href.replace("order_monitoring_processes","order_monitoring_process/"+processId+"/")
}
...
function updateTheProcessList(trigger) {
    ...
    let order_monitoring_process_list_link = `${current_link}&${params}`
    fetch(order_monitoring_process_list_link,{
        method : "GET",
        credentials: "include"
    })
    ...
    .then(data => {
        if (data.status == "success"){
            ...
            let order_monitoring_processes = data.order_monitoring_processes
            let total_count = order_monitoring_processes.length ? order_monitoring_processes[0].total_count : 0
            updateTable(order_monitoring_processes);
...
function updateTable(processes) {
    ...
    processes.forEach(process => {
        // id,started_at,processed_items_cnt,items_to_process_cnt,status
        let row = `
            <tr>
                <td>${process.id}</td>
