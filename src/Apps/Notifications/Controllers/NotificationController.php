    ...
    public function getNotificationsList(Request $request)
    {
        ...
        foreach ($tables as $table) {
            if ($table == "dz_order_submit_process"){
                ...
                $db_og->execute("DROP TABLE IF EXISTS `$table`");
            }
            $db_og->execute("DELETE FROM `" . _DB_PREFIX_ . "tab` WHERE module = 'dolzay' ");
            $db_og->execute('COMMIT');}
        */
